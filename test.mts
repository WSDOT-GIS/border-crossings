import {
  getCurrentBorderCrossingInfo,
  createWaitTimesPageUrl,
  CrossingTypes,
} from "./index.mjs";
...
const crossingTestPromise = (async () => {
  ...
  for (const crossing of crossings.filter(
    (c) => c.border === "Canadian Border"
  )) {
    assert.equal(
      crossing.date instanceof Date,
      true,
      `Expected ${crossing.date} to be a ${Date}`
    );
    assert.match(
      crossing.port_number,
      /\d+/,
      "expected port number to be only digits"
    );
  }
...
const waitTimesPagePromise = (async () => {
  ...
  const waitTimesUrl = createWaitTimesPageUrl(
    "02300402",
    CrossingTypes.Passenger
  );
